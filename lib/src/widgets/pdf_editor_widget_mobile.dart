...
class PdfEditorWidget extends StatefulWidget {
  ...
  @override
  _PdfEditorWidgetMobileState createState() => _PdfEditorWidgetMobileState();
}

/// State for the mobile Html editor widget
///
/// A stateful widget is necessary here to allow the height to dynamically adjust.
class _PdfEditorWidgetMobileState extends State<PdfEditorWidget> {
  ...
  initPdfEditor(InAppWebViewController controller) async {
    var viewerJs = await rootBundle.loadString('packages/pdf_editor/assets/web/viewer.js');
    viewerJs = '<script>' + viewerJs + '</script>';
    var htmlString = await rootBundle.loadString(filePath);
    htmlString = htmlString.replaceFirst('<script src="viewer.js"></script>', viewerJs);
    fileData = htmlString;
    controller.loadData(data: fileData);
  }
              ...
              Expanded(
                child: InAppWebView(
                  ...
                  onWebViewCreated: (InAppWebViewController controller) async {
                    ...
                    await initPdfEditor(controller);
                  },
                  ...
                  onLoadStop:
                    ...
                    if (url.contains(filePath)) {
                      ...
                      if ((Theme.of(context).brightness == Brightness.dark ||
                        ...
                        var darkCSS =
                            '<link href=\"${(widget.htmlEditorOptions.filePath != null ? "file:///android_asset/flutter_assets/packages/pdf_editor/assets/" : "") + "summernote-lite-dark.css"}\" rel=\"stylesheet\">';
                        await controller.evaluateJavascript(
