import * as pdfLib from '../pdf-lib/pdf-lib.js';
/**
...
const PDFViewerApplication = {
  ...
  async logData() {
        ...
        try {
          ...
          window.chrome.webview.postMessage(JSON.stringify({'pdf_data': data}));
//          const blob = new Blob([data], {
//            type: "application/pdf"
//          });
//          await this.downloadManager.download(blob, url, filename);
...
//  file = "C:\\src\\ProvaEditOF.pdf";//params.get("file") ?? _app_options.AppOptions.get("defaultUrl");
//  file = "https://raw.githubusercontent.com/mmasdivins/html-editor-enhanced/windows/lib/pdf-lib_form_creation_example.pdf";//params.get("file") ?? _app_options.AppOptions.get("defaultUrl");
...
async function savePdf() {
    console.log("Document: ", PDFViewerApplication.pdfDocument);
    ...
    for (var i = 1; i <= numPages; i++) {
      ...
      // get the form fields for the page
//      var resPage = {};


      var fields = await page.getAnnotations();
      console.log("form fields: ", fields);

      var storage = PDFViewerApplication.pdfDocument.annotationStorage;
      console.log("storage: ", storage);
      console.log("storage getAll: ", storage.getAll());
      var formD = storage.getAll();
      console.log("storage serializable: ", storage.serializable);

      for (var j = 0; j < fields.length; j++) {
          var element = fields[j];

//          var val = PDFViewerApplication.pdfDocument.annotationStorage.getOrCreateValue(element.id);
          var val = storage.getValue(element.id, "");
          console.log("========");
          console.log("element: ", element);
          console.log("name: ", element.fieldName);
          console.log("val: ", val);
          console.log("storage getRawValue: ", storage.getRawValue(element.id));
//          console.log("storage getValue: ", formD[element.id]);
          console.log("========");
          if (element.radioButton) {
              if (val.value) {
                  ...
                  console.log("rb: ", element.fieldValue);
                  result[element.fieldName] = element.fieldValue;
              }
          }
          else if (element.checkBox) {
              console.log("reading checkbox fieldname: ", element.fieldName);
              console.log("reading checkbox: ", element.fieldValue);
//                  result[element.fieldName] = val.value ? "On" : "Off";
              if (val.value !== undefined){
                ...
                if (element) {
                  const exportValue = element.exportValues ? element.exportValues[0] : 'Yes';
                  console.log("exportValue: ", exportValue);
                  if (element.fieldValue === exportValue) {
                    checkboxValue = true;
                  }
                }
                result[element.fieldName] = checkboxValue;
              }
          }
          else if (element.combo || (Array.isArray(element.options) && element.options.length > 0 && !element.combo)){
            if (val.value !== undefined){
                result[element.fieldName] = val.value;
            } else {
                result[element.fieldName] = element.fieldValue;
...
//    console.log("pdfLib: ", pdfLib);
    const { PDFDocument, StandardFonts, PDFBoolean, PDFName } = PDFLib;
//    console.log("PDFDict: ", pdfLib.PDFDict);
...
//      form.getRadioGroup('Group2').select('Choice2');
...
//        field.setText(result[fieldName]);
//        form.updateFieldAppearances(field);

            const field = form.getField(fieldName);
            console.log("field: ", field);
            console.log("fieldName: ", field.getName());
            console.log("constructorName: ", field.constructor.name);

            if (field.constructor.name === 'PDFTextField') { // for text field
              console.log("A text field: ", result[fieldName]);
              if (result[fieldName] !== undefined){
                field.setText(result[fieldName]);
//              form.updateFieldAppearances(field);
              }
            } else if (field.constructor.name === 'PDFCheckBox') { // for checkbox field
              console.log("A Checkbox field: ", result[fieldName]);
...
//              form.updateFieldAppearances(field);
            } else if (field.constructor.name === 'PDFDropdown') { // for checkbox field
                console.log("A Dropdown field: ", result[fieldName]);
                if (result[fieldName] !== undefined){
                    field.select(result[fieldName]);
                }
            } else if (field.constructor.name === 'PDFOptionList') { // for checkbox field
                console.log("A PDFOptionList field: ", result[fieldName]);
                if (result[fieldName] !== undefined){
                    field.select(result[fieldName]);
                }
            } else if (field.constructor.name === 'PDFRadioGroup') { // for checkbox field
                console.log("A PDFRadioGroup field: ", result[fieldName]);
                if (result[fieldName] !== undefined){


                    const options = field.getOptions();
                    console.log("PDFRadioGroup options: ", field.getOptions());
                    console.log("PDFRadioGroup getSelected 1: ", field.getSelected());
//                    const selectedOption = options[result[fieldName]];
                    const selectedOption = result[fieldName];
//                    const selectedOption = 'Falcon Heavy';
                    console.log("PDFRadioGroup selectedOption: ", selectedOption);
                    console.log("field.acroField: ", field.acroField.getOnValues());

                    if (selectedOption !== undefined && selectedOption !== null){
//                        var rg = form.getRadioGroup(fieldName);
//                        console.log("PDFRadioGroup options: ", rg);
//                        field.acroField.setValue(field.acroField.getOnValues()[0]);
...
//                        form.markFieldAsDirty(field.ref);
                    ...
                    //                     console.log("acroField: ", field.acroField);
                    //                     const widgets = field.acroField.getWidgets();
                    //                     field.updateAppearances();
                    //                     for (const widget of widgets) {
                    //                        console.log("widget: ", widget);
                    //                        widget.setAppearanceState(selectedOption);
                    //                     }

                    //                    const widgets = pdfDoc.getPages()[0].getWidgetsOfType(PDFWidgetAnnotationFlags.Radio);
                    //                    for (const widget of widgets) {
                    //                      if (widget.radioButton.name === radioGroup.getName()) {
                    //                        if (widget.radioButton.value === 'value') {
                    //                          widget.radioButton.setAppearanceState('On');
                    //                        } else {
                    //                          widget.radioButton.setAppearanceState('Off');
                    //                        }
                    //                        widget.updateAppearances();
                    //                      }
                    //                    }


                    //                    console.log("PDFRadioGroup getSelected 2: ", field.getSelected());
...
//    const pdfBytes2 = await pdf.saveDocument(pdf.annotationStorage);
    window.chrome.webview.postMessage(JSON.stringify({'pdf_data': pdfBytes}));

//    PDFViewerApplication.logData();
    console.log("Hem provat de guardar");
}
